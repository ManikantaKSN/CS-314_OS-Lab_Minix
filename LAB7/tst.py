...
with ThreadPoolExecutor(max_workers=8) as executor:
    futures = [run_relocation(s, l) for s in range(0, 1) for l in range(0, 1025, 15)]
    for l, future in zip(range(0, 1025, 15), futures[::1]):
        y = future.result()
        x_values.append(l)
        print(y)
        y_values1.append(y/100)
        print(l)

with ThreadPoolExecutor(max_workers=8) as executor:
    futures = [run_relocation(s, l) for s in range(1,2) for l in range(0, 1025, 15)]
    for l, future in zip(range(0, 1025, 15), futures[::1]):
        y = future.result()
        print(y)
        y_values2.append(y/100)
        print(l)

with ThreadPoolExecutor(max_workers=8) as executor:
    futures = [run_relocation(s, l) for s in range(2, 3) for l in range(0, 1025, 15)]
    for l, future in zip(range(0, 1025, 15), futures[::1]):
        y = future.result()
        print(y)
        y_values3.append(y/100)
        print(l)
        
with ThreadPoolExecutor(max_workers=8) as executor:
    futures = [run_relocation(s, l) for s in range(3, 4) for l in range(0, 1025, 15)]
    for l, future in zip(range(0, 1025, 15), futures[::1]):
        y = future.result()
        print(y)
        y_values4.append(y/100)
        print(l)
