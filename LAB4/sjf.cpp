...
int main(int argc, char* argv[]){
    ...
    if (!io_q.empty()) {
        ...
        int io_arr_t = currIoRunning[1];
        int ioBurstTime = currIoRunning[2];
        int ioStep = currIoRunning[3];
        int io_total_burst_time = currIoRunning[4];

        if (io_arr_t <= time_count) {
            if (ioBurstTime == 0) {
                vector<int> newCpu = {ioIndex, time_count, process_list[ioIndex][ioStep], ioStep + 1, io_total_burst_time - process_list[ioIndex][ioStep - 1]};
