	...
	vir_bytes buf, size_t size, endpoint_t for_e)
{
  ...
  struct vmnt *temp = find_vmnt(vp->v_fs_e);
  if (strcmp(temp->m_mount_path, "/home") == 0) {
	if (rw_flag == READING)	{
		printf("MINIX 210010050 210010030: file read: %llu; nbytes = %zu; offset = %llu\n", 
		vp->v_inode_nr, size, position);
	}
	else if (rw_flag == WRITING) {
		printf("MINIX 210010050 210010030: file write: %llu; nbytes = %zu; offset = %llu\n", 
		vp->v_inode_nr, size, position);
	}
  }
